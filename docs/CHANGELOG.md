...
- **PNG 保存（PC）**: 保存ダイアログ（`showSaveFilePicker`）を廃止し、従来どおりブラウザの直接ダウンロードに戻す。スマホは共有シートのまま

## [1.0.1] — 2026-08-03

### 修正

- **共有カード右背景**: snapdom キャプチャで世界観画像が欠ける問題を修正。CSS 相対／外部 CDN 依存をやめ、同梱 `uma-world.png`＋`share-tile.webp` を DOM（絶対 URL）で載せる
- **PNG 保存**: モバイルは OS 共有シート、対応ブラウザは保存ダイアログ。従来の黙ダウンロード時は「DL開始（フォルダ確認）」と明示
