...
- **状態割引トグル**: 合計SPバーで `OFF → 勉強家 ON → 切れ者 ON` を切替。勉強家は4%・切れ者は10%のスキルPt軽減で、両者は重複しない
- **保存・共有カード対応**: 状態割引を設計メモリ／セッション復元と共有カードへ反映。従来の「切れ者ON」保存データも互換復元

### 変更

- **公開棚接続**: 起動時に `umamusume-data`（第1波 JSON／カード画像）を取得。失敗時は同梱 `data/` `assets/` に戻す。`?hub=local` で同梱のみ、`?hub=remote` でハブ必須
- **サポカ選択は対象分のみ**: 「イベント対応」トグル OFF＝全件編成を廃止
- **スマホ固定バー**: 状態割引チップを固定幅・短縮表示（`割引 OFF` / `勉強家 ON` / `切れ者 ON`）にし、375px幅で継承チップが折り返さないよう調整
- **JS/CSS キャッシュバス**: `1.0.7`
