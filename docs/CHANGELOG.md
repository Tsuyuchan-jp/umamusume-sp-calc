...
- **PNG 保存（Pages）**: 大きな共有カードで `canvas.toDataURL` がブラウザ上限に当たり DL できない問題を、Blob URL 経由の保存に変更して修正

### 変更

- **v1.0.1 取り消し**: 共有カード右背景・PNG保存導線の変更をロールバック（v1.0.0 相当に復帰）。PNG は従来の直接ダウンロードのみ
