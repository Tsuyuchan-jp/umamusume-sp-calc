...
| UI で手動 OFF にしたスキル | 合計から除外。レギュ変更でも維持。編成変更で表示行 ID が変わっても、同一チェーン／グループ内なら表示行へ引き継ぐ |
| 結果のバ場／距離／作戦レギュで非互換と判定されたスキル | 合計から除外（表示行は残す）。**互換判定は表示行（最上段 skillId）の `activation` のみ**（チェーン下位は OR しない）。セグメント選択で即反映。手動 OFF は維持 |
| 結果パネルスキル数表示 | 見出し右 `ONスキル数 N/M`。N＝合計に含まれるスキル本数、M＝一覧のスキル本数。通常行は1行＝1本。継承固有は一覧1行のままオプション個数分を本数に加算（継承行に ON トグルは無く、オプション ON 時は ON/全件とも加算）。実機確認済み（v0.1.10） |
| ONスキルコピー | 見出し右ボタン。ON の表示行 `name` をカンマ区切り1行でクリップボードへ。継承固有・OFF 行は含めない。実機確認済み |
| 継承固有（オプションOFF） | 加算しない |
...
- 再生成: `npm run extract:events` → `npm run apply:events`（手順: [GAME_UPDATE_RUNBOOK.md](../ops/GAME_UPDATE_RUNBOOK.md)、設計経緯: [archive/EVENT_EXTRACT_DESIGN.md](../archive/EVENT_EXTRACT_DESIGN.md)）
