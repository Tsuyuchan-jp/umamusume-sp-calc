...
- 入力: mdb + U-tools（**優先37種のみ**）
- 出力: `data/events.extracted.json` と `data/events.diff-report.json`
- ゴールデン: 現行 `events.json` とスキル集合ベースで比較
- 成功: 旧11種は差分ゼロ〜許容リストのみ。26種は U-tools 正で手メンテ誤記を検出できること

### Phase B — 優先37種の手メンテ置換（**最終ゴール**）

- `events.json` を抽出結果で置換
- `data/events.id-aliases.json` を1バージョン残し、旧 id → 新 id を記録してから破棄可
- `prioritySupportNames` は維持（37種限定の UI 表示のまま）
...
| `data/events.json` | アプリ正本 | Phase B で置換 |
...
**Composer 2.5**（U-tools パースで構造が読めないときだけ Medium）

### タスク

1. U-tools 1枚分の SSR JSON 形状を固定 → パース仕様メモ
2. `extract_support_events.mjs` — mdb 紐付け + U-tools fetch/parse（**37種限定**）
3. `event_selection` / ラベル整形へ接続
4. ゴールデン比較スクリプト — 37種 diff レポート
5. Phase B: `events.json` 置換 + `events.id-aliases.json`
6. docs 更新（`DATA.md` / `ARCHITECTURE.md` / `HANDOFF` / `ROADMAP`）
