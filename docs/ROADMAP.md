...
- [x] **v0.1.13** — ゲーム更新追従（DMM mdb）・優先 +2種（キセキ/夏空アーモンド・計40種）・GameWith 一時イベント6件・**実機確認済み**

## 未完了 / 残作業

| 項目 | 状態 | メモ |
|------|------|------|
| UX 改善 | 進行中 | 作業キューは [TODO.md](./TODO.md)。画像（サポカ縦40＋育成全264）済。由来・初期6枚・スクショ・質感パスは未着手 |
...
5. ~~継承固有をスキル数に個数分カウント~~ **済**（v0.1.10・実機 OK）
6. ~~UX 思想の文書化（[UX_PHILOSOPHY.md](./UX_PHILOSOPHY.md)）~~ **済**（2026-08）
7. ~~PC 編成ダッシュボード・視覚選択（6枠ピッカー・前提チップ）~~ **済**（2026-08）
7a. ~~カード画像 Phase 1（優先40 + 育成全264・`assets:extract`/`import`）~~ **済**（2026-08・[ASSETS.md](./ASSETS.md) / [TODO.md](./TODO.md)）
8. 結果表の由来表示強化 — **Open**（[TODO.md](./TODO.md) P0）
9. 初期値を常用デッキ6枚に揃える — **Open**（[TODO.md](./TODO.md) P0）
10. ~~メモリ（`designSnapshot.js` 往復＋localStorage）~~ **済**（v0.1.14）。スクショは未着手（[TODO.md](./TODO.md) P2）
11. （需要が明確なら）プリセット — 現状スコープ外
12. 全体デザイン質感パス — **Open**（[TODO.md](./TODO.md) P1）
