...
- [x] **v0.1.8** — 含める ON スキル名のクリップボードコピー（カンマ区切り・継承除外）

## 未完了 / 残作業

| 項目 | 状態 | メモ |
|------|------|------|
| UX 改善 | 任意 | 結果の由来表示、初期デッキ6枚化など。プリセットは当面スコープ外。※レギュ絞込適用ボタン v0.1.7・説明書 v0.1.5・発動条件タグ絞込 v0.1.6 で実装済み |
| 回帰テスト拡充 | 一部済 | デフォルト編成・リンク白/金・発動条件タグ絞込を `npm test` に追加（2026-07） |
