...
| 項目 | 状態 | メモ |
|------|------|------|
| UX 改善 | 進行中 | 思想・視覚編成・メモリ・優先枠画像 Phase1 済（2026-08）。由来・初期6枚・スクショ・質感パスは未着手 |
...
7a. ~~カード画像 Phase 1（優先40 + 107703・`assets:extract`/`import`）~~ **済**（2026-08・[ASSETS.md](./ASSETS.md)）
8. 結果表の由来表示強化
9. 初期値を常用デッキ6枚に揃える
10. ~~メモリ（`designSnapshot.js` 往復＋localStorage）~~ **済**（v0.1.14）。スクショは未着手
