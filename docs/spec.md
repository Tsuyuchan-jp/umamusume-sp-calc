...
| 結果パネルスキル数表示 | 見出し右 `スキル数 N/M`。N＝合計に含まれるスキル本数、M＝一覧のスキル本数。通常行は1行＝1本。継承固有は一覧1行のままオプション個数分を本数に加算（継承行に「含める」チェックは無く、オプション ON 時は ON/全件とも加算） |
