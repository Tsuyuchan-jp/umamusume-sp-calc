...
- **除外**: 一覧からスキルを外して合計から落とせる（対人レギュ等）。結果の **バ場／距離／作戦** 絞込で非互換スキルを一括除外も可（絞込変更で手動除外はリセット）
...
- **結果スキル絞込**: バ場／距離／作戦（未選択＝全 ON）。`skills.json` の `activation` で互換判定。絞込変更時は `excludedSkillIds` を再計算（ゲームのスキル取得画面と同様に手動「含める」もリセット）
