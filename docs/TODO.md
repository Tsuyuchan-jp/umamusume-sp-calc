...
**デュアルUIほぼ安定** — 継承詳細編集UI・シナリオ自動コンパクト・ヘッダー幅は保留。次は Round 1 か仕上げパス。
...
- [x] **H5モック改訂** — A/B/C 反映・自動計上Bは件数チップ→モーダル（インライン折りたたみしない） `2026-08-02`
- [x] **デュアルUIを app へ移植**（第2弾済 `2026-08-02`）— A要約ダイアログ・継承バー上展開・H5トーン。スプリット左ペイン詳細はダイアログで代替
- [ ] **UI Round 1**（移植と並行可）— ピッカー選択強調・タイプ絞込バグ
...
- [ ] **シナリオ自動の縦コンパクト**／**ヘッダー幅の中央寄せ** — 見た目仕上げパスで一括可
