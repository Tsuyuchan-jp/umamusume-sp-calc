...
**H5 vs app ギャップ調査済** — P0/P1 優先リストを人間確認後、スプリット H1 パリティから実装。
...
- [ ] **スプリット左 2×3 ワイドドック** — split 時も 6 列のまま → モック相当の 2×3＋育成横並びへ（計算ロジック不変）
- [ ] **スプリット上部コマンド** — 合計＋前提を上へ集約、下端バーは split 時非表示（モック `.s-cmd`）
- [ ] **スプリット左 A 詳細ペイン** — 現状ダイアログ代用 → 左ドック内ペイン（ギャラリーはダイアログ維持可）

### P1

- [x] 2026-08-02 **デュアル移植ギャップ調査** — 結果は [product/design-overhaul-handoff.md](./product/design-overhaul-handoff.md) §2b
- [ ] **ギャラリー／全体の見た目パリティ** — パネル bg.webp 質感・足元帯・カード密度の H5 寄せ（調査 §2b #9）
- [ ] **UI Round 1**（並行可）— ピッカー選択強調・タイプ絞込 `keepId` バグ
- [ ] **B 自動の方針確認** — UX.md（件数チップ→モーダル）に戻すか、現行（列下＋シナリオ足元インライン）を正とするか
...
- [ ] **シナリオ自動の縦コンパクト**／**ヘッダー幅の中央寄せ** — 見た目仕上げパスで一括可（B 方針と連動可）
