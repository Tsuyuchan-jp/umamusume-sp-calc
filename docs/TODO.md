...
**P0 スプリット H1 移植済** — 実機確認待ち。B 方針はスプリット見た目確認後に検討。
...
（なし — スプリット H1 3件は実装済・実機確認待ち）

### P1

- [ ] **スプリット H1 実機確認** — 2×3ドック／上部コマンド／左Aペインの受け入れ
- [ ] **ギャラリー／全体の見た目パリティ** — パネル bg.webp 質感・足元帯・カード密度の H5 寄せ（調査 §2b #9）
- [ ] **UI Round 1**（並行可）— ピッカー選択強調・タイプ絞込 `keepId` バグ
- [ ] **B 自動の方針確認**（スプリット見た目が整ってから）— UX.md（モーダル）に戻すか、現行（列下＋シナリオ足元インライン）を正とするか。モーダル削除の是非は split 正が見えないと判断できない
