...
**スプリット足元・上部バー手入れ済** — 実機でリンク/RMJチップと幅揃えを確認。
...
- [ ] **スプリット H1 実機確認** — 2×3ドック／上部コマンド幅／リンク・RMJチップ／左Aペイン
- [ ] **ギャラリー／全体の見た目パリティ** — パネル bg.webp 質感・カード密度の H5 寄せ（調査 §2b #9）
- [ ] **UI Round 1**（並行可）— ピッカー選択強調・タイプ絞込 `keepId` バグ
- [ ] **B 自動の方針確認**（スプリット見た目が整ってから）— UX.md（モーダル）に戻すか、現行（列下＋シナリオ足元）を正とするか
