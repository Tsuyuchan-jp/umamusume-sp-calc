...
**P2 スマホ対応** — 対話中。レイアウト方針合意済（狭幅=ギャラリー固定・自動削除）。残り受け入れ条件を順に確定
...
- [~] **スマホ対応**（対話中・実装前） — レイアウト: 狭幅ギャラリー固定・PCは手動切替のみ・**自動（幅）削除**（[UX.md](./product/UX.md#レイアウト切替合意2026-08-03)）。境界px・拡張（タップ／表等）は対話継続 → 合意後に受け入れ条件を本行へ追記して実装
