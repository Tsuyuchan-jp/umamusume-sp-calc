...
**P2 編成スクショ** — 本実装（正本モック v28.1 を完璧に移植）
...
- [ ] **編成スクショ機能**（モック採用済・本実装待ち・2026-08-02）  
  - メモリ（localStorage）は済。共有用の画像書き出しは未着手  
  - 要件メモ: [product/screenshot-requirements.md](./product/screenshot-requirements.md)  
  - **正本モック**: [mockups/screenshot-share-hero.html](./product/mockups/screenshot-share-hero.html)（**v28.1 採用**）  
  - **移植指示**: モックのデザインを完璧に移植する（簡略化・別案化は不可。差分はユーザー確認）  
  - 用途第一: 攻略記事貼付。専用横長カード。コピー優先＋保存。モード非依存  
  - 次: **本実装**（見た目＝モック忠実移植 → 実データ配線 → コピー／保存）  
  - 受け入れ: 正本モックと見た目同等＋現在設計を1枚として保存／コピーできる
