...
**デュアル移植は見た目パリティ未完** — 次チャットは H5 vs app のギャップ調査から（実装は調査・優先付けの後）。
...
- [ ] **デュアル移植ギャップ調査** — `hybrid-h5-dual.html` vs `app/` を比較し未移植・中途半端を一覧化→優先付け（人間確認後に実装）
- [ ] **デュアルUI見た目パリティ**（調査結果に従う）— 計算ロジックは触らない
- [ ] **UI Round 1**（並行可）— ピッカー選択強調・タイプ絞込バグ
