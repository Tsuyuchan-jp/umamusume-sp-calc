...
**P2 スマホ対応** — 対話中。レイアウト: ≤1199 ギャラリー固定・≥1200 手動切替・自動削除。次は modebar
...
- [~] **スマホ対応**（対話中・実装前） — レイアウト: **≤1199px ギャラリー固定**・≥1200px 手動切替のみ・**自動（幅）削除**（[UX.md](./product/UX.md#レイアウト切替合意2026-08-03)）。modebar・拡張（タップ／表等）は対話継続 → 合意後に受け入れ条件を本行へ追記して実装
