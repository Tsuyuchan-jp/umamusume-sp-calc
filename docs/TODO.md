...
**実機デザイン指摘6点を Open 化** — 確認後に P0 から実装（[product/design-polish-backlog.md](./product/design-polish-backlog.md)）。
...
- [ ] **ヘッダー安定化＋見た目刷新** — 切替で出現／消滅しない。H5 クロム寄せ（backlog #1）
- [ ] **スプリット左ドックのゾーン分け** — サポカ選択とイベント詳細の境界を明確に（backlog #3）
- [ ] **世界観背景に芝を入れる** — `background-position` 等（backlog #5）
- [ ] **パネル幾何タイル（bg.webp）適用** — モック `--panel`（backlog #6）

### P1

- [ ] **結果作業台の見た目パリティ** — 含めるトグル・レギュ絞込等。列／要件は維持（backlog #2）
- [ ] **カードピッカー刷新** — 0からデザイン選定後に実装（backlog #4・案 P-A/B/C）
...
- [x] 2026-08-02 **実機デザイン指摘6点を backlog 化** — ヘッダー／結果枠／左ドック／ピッカー案／芝／幾何（必須なし判定を上書き）
- [x] 2026-08-02 **デザイン残件の再判定** — 骨格向け「必須なし」→ 同日の実機6点で差し替え
- [x] 2026-08-02 **B方針確定** — モーダル不採用。サポカ自動=列下、シナリオ自動=足元折りたたみを本線
- [x] 2026-08-02 **UI Round 1** — ピッカータイプ絞込 `keepId` すり抜け修正・選択強調（※全面刷新は Open）
- [x] 2026-08-02 デュアル骨格を構造OKと確定
- [x] 2026-08-02 スプリット足元UI刷新・P0 スプリット H1・ギャップ調査・FB反映・移植第1–2弾
- [x] 2026-08-02 Visual Foundation／質感パス／モック v1〜H5
- [x] 2026-08-01 由来表示強化・ピッカー検索・初期デッキ・docs 組み直し・サポカ縦カード・育成264同梱
