...
> ウマ娘 SP 計算ツール（**v1.0.6**）。**トレセン軒シナリオのみ**・**ガチ想定**（クラシック大盛況・シニア超盛況・育成終了スキル固定）・**優先42サポカのイベント**対応（U-tools+mdb 抽出正本）。ゲーム更新後はデータが古くなる場合があります。非公式・非商用です。

### 使い方の流れ

1. 広い画面では上部のレイアウト切替（ギャラリー／スプリット）で画面構成を選ぶ（狭い画面はギャラリー固定）
2. 育成ウマ娘を選ぶ（初期値あり）。編成タイトルは見出し下で編集可
3. サポカを最大6枚選ぶ（初期は**イベント対応42種**に絞込。全件表示は「イベント対応のみ」を外す）
...
- **イベントは優先42サポカ** — 全サポカ網羅ではない（[一覧](docs/product/PRIORITY_SUPPORTS.md)）。新規追加は最新の課金必須 SSR を原則とする
