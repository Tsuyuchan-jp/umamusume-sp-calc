...
> ウマ娘 SP 計算ツール（**v0.1.11**）。**トレセン軒シナリオのみ**・**ガチ想定**（クラシック大盛況・シニア超盛況・育成終了スキル固定）・**優先38サポカのイベント**対応（U-tools+mdb 抽出正本）。ゲーム更新後はデータが古くなる場合があります。非公式・非商用です。

### 使い方の流れ

1. 育成ウマ娘を選ぶ（初期値あり）
2. サポカを最大6枚選ぶ（初期は**イベント対応38種**に絞込。全件表示は「イベント対応のみ」を外す）
3. イベント／シナリオ（リンク・ラーメン等）を必要なら調整
4. 画面下部の**合計 SP**を確認（除外・切れ者・継承固有・**トレヒントLv**も可）
...
- **イベントは優先38サポカ** — 全サポカ網羅ではない（[一覧](docs/PRIORITY_SUPPORTS.md)）。新規追加は最新の課金必須 SSR を原則とする
...
イベント対応サポカ38種の一覧は [docs/PRIORITY_SUPPORTS.md](docs/PRIORITY_SUPPORTS.md)（JSON: [data/priority-supports.json](data/priority-supports.json)）。
