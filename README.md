...
> ウマ娘 SP 計算ツール（**v1.0.1**）。**トレセン軒シナリオのみ**・**ガチ想定**（クラシック大盛況・シニア超盛況・育成終了スキル固定）・**優先40サポカのイベント**対応（U-tools+mdb 抽出正本）。ゲーム更新後はデータが古くなる場合があります。非公式・非商用です。
...
7. 前回の続きは自動復元。名前付き保存はヘッダー「メモリ」。共有は広い画面はモードバー右端、狭い画面は結果見出し横の画像コピー／PNG。初期に戻すときは編成見出し右の「リセット」
