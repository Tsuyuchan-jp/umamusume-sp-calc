...
> ウマ娘 SP 計算ツール（**v1.0.7**）。**トレセン軒シナリオのみ**・**ガチ想定**（クラシック大盛況・シニア超盛況・育成終了スキル固定）・**優先42サポカのイベント**対応（U-tools+mdb 抽出正本）。ゲーム更新後はデータが古くなる場合があります。非公式・非商用です。

### 使い方の流れ

1. 広い画面では上部のレイアウト切替（ギャラリー／スプリット）で画面構成を選ぶ（狭い画面はギャラリー固定）
2. 育成ウマ娘を選ぶ（初期値あり）。編成タイトルは見出し下で編集可
3. サポカを最大6枚選ぶ（**イベント対応分のみ**。検索・SSR・タイプでも絞れる）
4. イベント／シナリオ（リンク・ラーメン等）を必要なら調整
5. **合計 SP**を確認（状態割引［OFF／勉強家／切れ者］・除外・継承固有・**トレヒントLv**も可。ギャラリーは下部、スプリットは上部）
...
- **イベントは優先42サポカ** — 選択できるのもその対象分のみ（[一覧](docs/product/PRIORITY_SUPPORTS.md)）。新規追加は最新の課金必須 SSR を原則とする
